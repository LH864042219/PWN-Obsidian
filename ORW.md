# 介绍 
`ORW(open,read,write)`类的题目一般出现在开启了`sandbox`的题目中。这类题目通常会禁用一些系统调用(例如execve之类的)，但一般会保留orw中的一些，虽然我们无法获取shell,但这时候我们可以利用这些系统调用来直接读取flag并输出。
...
# 特殊
## orw缺w
这种情况下，若是找不到任何可以用于替代write的函数，则可以使用侧信道爆破的方法来逐位爆破出flag的值