...
这里我们free后可以看到free了两个chunk，一个是大小固定的chunk，另一个是我们修改了size后的chunk大小，所以我们再申请一个大小的0x50(因为还要算上size域这些所以要小0x10)的chunk,那么就可以覆盖到下一个chunk。
![[Pasted image 20250323150324.png]]
这里可以看到新申请的chunk以及我们申请的大小为0x50的chunk都申请成功，且我们可以修改两个chunk的指针，把指向chunk的指针修改为任意函数的got后执行show就可以泄漏libc，然后再用edit就可以修改该函数的got。
